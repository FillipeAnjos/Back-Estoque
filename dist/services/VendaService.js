...
var _VendaRepositories = require("../repositories/VendaRepositories");

var _FechamentoService = require("./FechamentoService");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

class VendaService {
  async salvarVenda(dados) {
    const vendaRepository = (0, _typeorm.getCustomRepository)(_VendaRepositories.VendaRepositories);
    const fechamentoService = new _FechamentoService.FechamentoService();
    const dataAtual = (0, _moment.default)(new Date()).format("YYYY-MM-DD"); // ------------------------------------------------------------ Validações ------------------------------------------------------------

    if (dados.itens.length == 0) {
      return {
        error: true,
        msg: "Sua lista de produtos está vazia, favor informar os produtos da venda."
      };
    }
    ...
    if (await duplicados) {
      return {
        error: true,
        msg: "Error: Você não pode adicionar dois produtos com o mesmo código."
      };
    }
    ...
    if (dados.modalidade.dinheiro) {
      ...
      return {
        error: true,
        msg: "Error: Você precisa informar qual a modalidade de pagamento."
      };
    }
    ...
    try {
      dados.valores.desconto = dados.valores.desconto == '' ? null : dados.valores.desconto; // Tabela de Vendas
      //      id - autoincrement
      //      idFechamento
      //      dados.id_user
      ...
      await Promise.all(dados.itens.map(async ele => {
        ...
        arrayUpdateQuantidade.push({
          id_produto: ele.id,
          qtd: quantidadeNova,
          qtd_saida: ele.unidade,
          valor: ele.valor
        });
      }));
      ...
      return {
        error: true,
        msg: "Ocorreu um erro ao tentar realizar a venda. Favor entrar em contato com o administrador do sistema. Desculpe!"
      };
    } finally {
  ...
  async buscarValorDia(idFechamento = null) {
    ...
    const vendaRepository = (0, _typeorm.getCustomRepository)(_VendaRepositories.VendaRepositories);
    const buscarDadosVenda = await vendaRepository.createQueryBuilder("vendas").where("id_fechamento = :id", {
      id: idFechamento
    }).getMany();
  ...
  async relatorioVendas(param) {
    ...
    dadosVendas = `${dadosVendas} order by ${ordenacao} ${ordenacaoordem}`; //console.log(dadosVendas);
